...
class TestUser(unittest.TestCase):
    ...
    def test_validate_lastname_success(self):
        ...
        for valid_lastname, expected_lastname in valid_lastnames:
            with self.subTest(lastname=valid_lastname):
                self.assertEqual(
                    self.user._validate_lastname(valid_lastname), expected_lastname
                )
    ...
    def test_validate_password_success(self):
        ...
        for valid_password in valid_passwords:
            with self.subTest(password=valid_password):
                self.assertEqual(
                    self.user._validate_password(valid_password), valid_password
                )
    ...
    def test_open_bank_account_success(self, mock_account_number):
        ...
        result = self.user.open_bank_account(
            bank=self.bank2, pin_code="123456", balance=1000
        )
    ...
    def test_close_bank_account_success(self):
        ...
        self.auth.login(
            user=self.user, email=self.user.email, password=self.user.password
        )

        self.user.withdraw(
            account_number="123456789", pin_code="123456", amount=1000, auth=self.auth
        )

        result = self.user.close_bank_account(
            account_number="123456789", pin_code="123456", auth=self.auth
        )
        bank_account = self.user.bank_accounts["123456789"]
    ...
    def test_get_total_balance_different_currency(self, mock_account_number):
        ...
        self.user.open_bank_account(
            bank=self.bank2, pin_code="654321", balance=500, currency="EUR"
        )
    ...
    def test_withdraw_success(self):
        ...
        result = self.user.withdraw(
            amount=100, account_number="123456789", pin_code="123456", auth=self.auth
        )
    ...
    def test_get_transactions_success(self):
        ...
        self.user.withdraw(
            amount=500, account_number="123456789", pin_code="123456", auth=self.auth
        )
    ...
    def test_get_transactions_by_date(self, mock_now):
        ...
        result = self.user.get_transactions_by_date(
            account_number="123456789",
    ...
    def test_unlock_account_success(self):
        ...
        result = self.user.unlock_account(
            account_number="123456789", pin_code="123456", auth=self.auth
        )
    ...
    def test_calculate_interest_success(self):
        ...
        result = self.user.calculate_intrest(
            days=10, account_number="123456789", auth=self.auth
        )
    ...
    def test_get_user_success(self):
        ...
        self.auth.login(
            user=self.admin, email=self.admin.email, password=self.admin.password
        )

        if self.user.id not in self.bank.users:
            self.bank.add_user(self.user)

        result = self.admin.get_user(
            user_id=self.user.id, bank=self.bank, auth=self.auth
        )
    ...
    def test_user_not_logged_in_errors(self):
        ...
        methods_to_test = [
            (
                "close_bank_account",
                self.user.close_bank_account,
                {"account_number": "123456789", "pin_code": "123456"},
            ),
            ...
            (
                "deposit",
                self.user.deposit,
                {"amount": 100, "account_number": "123456789", "pin_code": "123456"},
            ),
            (
                "withdraw",
                self.user.withdraw,
            ...
            (
                "get_transactions",
                self.user.get_transactions,
                {"account_number": "123456789"},
            ),
            (
                "calculate_interest",
                self.user.calculate_intrest,
                {"days": 30, "account_number": "123456789"},
            ),
        ]
    ...
    def test_account_not_found(self):
        ...
        methods_to_test = [
            (
                "close_bank_account",
                self.user.close_bank_account,
                {"account_number": "987654321", "pin_code": "123456"},
            ),
            ("get_balance", self.user.get_balance, {"account_number": "987654321"}),
            (
                "deposit",
                self.user.deposit,
                {"amount": 100, "account_number": "987654321", "pin_code": "123456"},
            ),
            (
                "withdraw",
                self.user.withdraw,
            ...
            (
                "calculate_interest",
                self.user.calculate_intrest,
                {"days": 30, "account_number": "987654321"},
            ),
        ]
        self.auth.login(
            user=self.user, email=self.user.email, password=self.user.password
